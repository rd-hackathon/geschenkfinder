...
def get_items(category: str):
    ...
    try:
        ...
        c.execute('CREATE TABLE IF NOT EXISTS "{}" (id NUMERIC UNIQUE, name TEXT, price TEXT, url TEXT, image TEXT)'.format(category))
    ...
    for x, item in enumerate(items_html):
        i = {
            ...
            'price': item.find('span', {'class': 'price'}).text.replace('\xa0', ' ')
        }
...
if __name__ == '__main__':
    with open('kategorien.txt', 'r', encoding='utf-8') as f:
        lines = f.readlines()
